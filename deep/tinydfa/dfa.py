...
#                random_projection /= np.sqrt(np.prod(random_projection.shape[1:]))
            ...
            for i, layer in enumerate(dfa_context.dfa_layers):
                ...
                random_projection = torch.mm(grad_output.reshape(-1, grad_size).to(dfa_context.rp_device),
                                         dfa_context.feedback_matrix[i])  # Global random projection
                if dfa_context.normalization:
                    random_projection /= np.sqrt(np.prod(random_projection.shape[1:]))
...
class DFA(nn.Module):
    def __init__(self, dfa_layers, normalization=True, rp_device=None, no_training=False,
                 feedback_points_handling=FeedbackPointsHandling.LAST, batch_dims=(0,), feedback_matrix=None):
    ...
    def forward(self, input):
        if not(self.initialized or self.no_training):
            ...
            if self.feedback_matrix is None:
                ...
                torch.manual_seed(0)
                self.feedback_matrix = torch.randn(len(self.dfa_layers),self.output_size, self.max_feedback_size, device=self.rp_device)
